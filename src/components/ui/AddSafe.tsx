...
export default function AddSafe(props: AddSafeProps) {
  ...
  const handleSave = () => {
    if (coords) {
      const prev = JSON.parse(localStorage.getItem("safeZones") || "[]");
      localStorage.setItem(
        "safeZones",
        JSON.stringify([...prev, { ...coords, comment }])
      );
      alert(
        `Zona segura guardada en: Lat: ${coords.lat}, Lng: ${coords.lng}\nComentario: ${comment}`
      );
      setShowForm(false);
      setComment("");
      setCoords(null);
          ...
          <div className="relative bg-white rounded-2xl shadow-xl max-w-sm p-8">
            ...
            <div className="text-center text-gray-800 text-xl mb-8">
              <p className="font-family-franklin">
                Estás a punto de marcar un lugar como zona segura. Puedes dejar
                una descripción sobre este lugar.
              </p>
            </div>
