...
export default function AddDanger(props: AddDangerProps) {
  ...
  const handleSave = () => {
    if (coords) {
      const prev = JSON.parse(localStorage.getItem("dangerZones") || "[]");
      localStorage.setItem(
        "dangerZones",
        JSON.stringify([...prev, { ...coords, comment }])
      );
      alert(
        `Zona peligrosa guardada en: Lat: ${coords.lat}, Lng: ${coords.lng}\nComentario: ${comment}`
      );
      setShowForm(false);
      setComment("");
      setCoords(null);
          ...
          <div className="relative bg-white rounded-2xl shadow-xl max-w-sm p-8">
            ...
            <div className="text-center text-gray-800 text-xl mb-8">
              <p className="font-family-franklin">
                Estás a punto de marcar un lugar como peligroso. Puedes dejar una
